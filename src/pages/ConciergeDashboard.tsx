    ...
    <div className="min-h-screen bg-gray-100 p-8">
      <div className="max-w-6xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          <h1 className="text-3xl font-serif font-bold text-[#3C5999]">Concierge Command</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filtered.map(profile => (
            <Card key={profile.id} className="overflow-hidden border-none shadow-md">
              <div className="bg-[#3C5999] p-4 flex items-center gap-4">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  ...
                  {profile.preferences_usage && profile.preferences_usage.length > 0 && (
                     <div className="col-span-2">
                        <h4 className="font-bold text-[#3C5999] flex items-center gap-2 mb-2">
                           <Coffee className="w-4 h-4" /> Prep Kits
                        </h4>
                        <div className="flex flex-wrap gap-2">
                          {profile.preferences_usage.map((k: string) => (
                            <span key={k} className="bg-[#3C5999] text-white px-2 py-1 rounded text-xs uppercase tracking-wide">
                              {k}
                            </span>
                          ))}
                        </div>
