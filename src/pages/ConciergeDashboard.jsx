            ...
            <Card key={profile.id} className="overflow-hidden border-none shadow-md">
              <div className="bg-[#3B5998] p-4 flex items-center gap-4">
                 <div className="w-12 h-12 rounded-full bg-white/20 flex items-center justify-center overflow-hidden">
                   {profile.photo_url ? (
                     <img src={`https://your-supabase-url.supabase.co/storage/v1/object/public/avatars/${profile.photo_url}`} className="w-full h-full object-cover" />
                   ) : (
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  ...
                  <div>
                    ...
                    <ul className="list-disc list-inside text-gray-600">
                      {(profile.preferences_amenities || []).map(a => <li key={a}>{a}</li>)}
                    </ul>
                  </div>
                  ...
                  <div>
                    ...
                    <ul className="list-disc list-inside text-gray-600">
                      {(profile.preferences_products || []).map(p => <li key={p}>{p}</li>)}
                    </ul>
                  </div>
